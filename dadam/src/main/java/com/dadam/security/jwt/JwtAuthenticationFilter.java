 ...
 * 
 *//**
	 * JWT 인증 필터 - JWT 토큰 검증 - 인증 정보 설정 - 권한 체크
	 * 
 ...
 * 
 * try { // 1. JWT 토큰 추출 String jwt = getJwtFromRequest(request);
 * 
 * // 2. 토큰 유효성 검증 if (StringUtils.hasText(jwt) &&
 * jwtTokenProvider.validateToken(jwt)) {
 * 
 * // 3. 사용자명 추출 String username = jwtTokenProvider.getUsernameFromToken(jwt);
 * 
 * // 4. UserDetails 로드 UserDetails userDetails =
 * userDetailsService.loadUserByUsername(username);
 * 
 * // 5. 인증 토큰 생성 UsernamePasswordAuthenticationToken authentication = new
 * UsernamePasswordAuthenticationToken(userDetails, null,
 * userDetails.getAuthorities());
 * 
 * // 6. 인증 정보 설정 authentication.setDetails(new
 * WebAuthenticationDetailsSource().buildDetails(request));
 * 
 * // 7. SecurityContext에 인증 정보 설정
 * SecurityContextHolder.getContext().setAuthentication(authentication); }
 * 
 * } catch (Exception e) { logger.error("JWT 토큰 처리 중 오류 발생", e); }
 * 
